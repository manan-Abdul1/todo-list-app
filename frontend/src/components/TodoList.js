...
function TodoList({ tasks, setTasks }) {
  ...
  const handleTaskDelete = async (taskId) => {
    try {
      ...
      await axios.delete(`http://localhost:5500/tasks/${taskId}`);
  ...
  const handleTaskStatusChange = async (taskId, completed) => {
    try {
      ...
      await axios.put(`http://localhost:5500/tasks/${taskId}`, { completed });
  ...
  const handleEditTask = async (editedTask) => {
    try {
      ...
      await axios.put(`http://localhost:5500/tasks/${editedTask._id}`, editedTask);
